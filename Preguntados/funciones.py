...
def cargar_preguntas(categoria):
    preguntas = []
    with open('preguntas.csv', newline='', encoding='utf-8') as archivo:
...
def modificar_opciones():
    """
    Permite al usuario modificar las opciones del juego mediante una interfaz de selección.
    """
    puntos_acierto = 10  # Valor por defecto
    puntos_error = -5    # Valor por defecto
    oportunidades = 3    # Valor por defecto
    tiempo_disponible = 30  # Valor por defecto
    
    while True:
        # Mostrar el menú de opciones modificables
        print("\nOpciones actuales:")
        print(f"1. Puntaje por acierto: {puntos_acierto}")
        print(f"2. Puntaje por error: {puntos_error}")
        print(f"3. Oportunidades: {oportunidades}")
        print(f"4. Tiempo por pregunta: {tiempo_disponible} segundos")
        print("5. Salir")

        # Solicitar la opción que el usuario desea modificar
        try:
            opcion = int(input("\n¿Qué opción deseas modificar? (1-5): "))
        except ValueError:
        ...
        if opcion == 1:
            # Modificar puntaje por acierto
            puntos_acierto = int(input("Introduce el nuevo puntaje por respuesta correcta: "))
            print(f"Puntaje por acierto actualizado a {puntos_acierto}.")
        
        elif opcion == 2:
            # Modificar puntaje por error
            puntos_error = int(input("Introduce el nuevo puntaje por respuesta incorrecta: "))
            print(f"Puntaje por error actualizado a {puntos_error}.")
        
        elif opcion == 3:
            # Modificar oportunidades
            oportunidades = int(input("Introduce la nueva cantidad de oportunidades: "))
            print(f"Cantidad de oportunidades actualizada a {oportunidades}.")
        
        elif opcion == 4:
            # Modificar tiempo por pregunta
            tiempo_disponible = int(input("Introduce el nuevo tiempo por pregunta (en segundos): "))
            print(f"Tiempo por pregunta actualizado a {tiempo_disponible} segundos.")
        
        elif opcion == 5:
            # Salir del menú
            print("Opciones modificadas correctamente. Regresando al menú principal.")
            break
        
        else:
            print("Opción no válida. Por favor, selecciona una opción entre 1")
...
def mostrar_categorias():
    ...
    while esperando_categoria:
        ...
        # Si no se ha seleccionado una categoría, seguir mostrando la pantalla actual
        pygame.display.update()  # Asegúrate de actualizar la pantalla continuamente
...
def reproducir_musica():
    if musica_activada:
        ...
        pygame.mixer.music.load('sonido_juego (1).mp3')
...
def verificar_respuesta(respuesta_usuario, pregunta):
    global puntos, vidas, respuestas_correctas, comodin_duplicar_puntos_usado
    ...
    if respuesta_usuario == respuesta_correcta:
        ...
        # Verificar si los puntos son mayores a 5 antes de restarlos
        if puntos > 5:
            puntos -= 5
        else:
            puntos = 0  # Asegurarse de que no se restan puntos si ya se está en 0
        vidas -= 1
        respuestas_correctas = 0
...
def iniciar_juego(preguntas):
    ...
    while vidas > 0:
        ...
        tiempo_limite = pygame.time.get_ticks() + 10000  # 10 segundos desde el
        ...
        while esperando_respuesta:
            ...
            mostrar_texto(f"{tiempo_restante}", 250, 40, NEGRO, fuente)
